...
struct InterfaceInfo(T, TCtx: UserContext=UserContext) if (is(T == class) || is(T == interface))
{
    @safe:
	...
	static assert (BaseInterfaces.length > 0 || is (T == interface),
		       "Cannot register type '" ~ T.stringof
		       ~ "' because it doesn't implement an interface");
	static if (BaseInterfaces.length > 1)
		pragma(msg, "Type '" ~ T.stringof ~ "' implements more than one interface: make sure the one describing the auto interface is the first one");
	...
	*/
	...
	static if (methodCount) static const StaticMethodInfo[methodCount] staticRoutes = computeStaticRoutes();
	else static const StaticMethodInfo[0] staticRoutes;
		...
		template Impl(size_t idx) {
			static if (idx < memberNames.length) {
				...
				static if (name.length != 0)
					alias Impl = TypeTuple!(Filter!(isDisabledMethod, MemberFunctionsTuple!(I, name)), Impl!(idx+1));
